...
import com.rainbowsea.yupicturebackend.manager.websocket.disruptor.PictureEditEventProducer;
import com.rainbowsea.yupicturebackend.manager.websocket.model.PictureEditActionEnum;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        // 注意:这里是 Map<Long, Set<WebSocketSession>> ，带一次插入的vaLue值是Set集合，第一次要 new Set 集合，不然就为 NULL
        pictureSessions.putIfAbsent(pictureId,ConcurrentHashMap.newKeySet());
        pictureSessions.get(pictureId).add(session);  // 添加 key,value
        // 构造响应，发送加入编辑的消息通知
        PictureEditResponseMessage pictureEditResponseMessage = new PictureEditResponseMessage();
        pictureEditResponseMessage.setType(PictureEditMessageTypeEnum.INFO.getValue());
        String message = String.format("用户 %s 加入编辑", user.getUserName());
        pictureEditResponseMessage.setMessage(message);
        pictureEditResponseMessage.setUser(userService.getUserVO(user)); // 脱敏后的数据
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        // 生产消息 根据消息类型处理消息（生产消息队列到 Disruptor 环形队列中）
    ...
    public void handleEnterEditMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws Exception {
        // 没有用户正在编辑该图片，才能进入编辑（因为同一时刻，只能有一个用户在编辑）
    ...
    /**
     * 广播给也在查看想要编辑该图片的所有用户(支持排除某个 Session(自己))
     * @param pictureId
     * @param pictureEditResponseMessage
     * @param excludeSession 排除的会话(排除的被广播的用户)
     * @throws Exception
     */
    private void broadcastToPicture(Long pictureId, PictureEditResponseMessage pictureEditResponseMessage,
        ...
        if(CollUtil.isNotEmpty(sessionSet)) {
            ...
            // 配置序列化：将 Long 类型转为 String，解决丢失精度问题(虽然我们配置了HTTP是精度丢失的配置，
            // 但是这里是 WebSocket 需要单独配置)
